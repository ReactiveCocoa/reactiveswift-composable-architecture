...
extension EffectProducer {
  ...
  /// ``EffectProducer/cancel(id:)-6hzsl`` to identify which in-flight effect should be canceled.
  ...
  /// Turns an effect into one that is capable of being canceled.
  ///
  /// A convenience for calling ``EffectProducer/cancellable(id:cancelInFlight:)-29q60`` with a
  ...
  /// An effect that will cancel any currently in-flight effect with the given identifier.
  ///
  /// A convenience for calling ``EffectProducer/cancel(id:)-6hzsl`` with a static type as the
  ...
  public static func cancel(ids: [AnyHashable]) -> Self {
    .merge(ids.map(EffectProducer.cancel(id:)))
  }

  /// An effect that will cancel multiple currently in-flight effects with the given identifiers.
  ///
  /// A convenience for calling ``EffectProducer/cancel(ids:)-1cqqx`` with a static type as the
  /// effect's unique identifier.
  ///
