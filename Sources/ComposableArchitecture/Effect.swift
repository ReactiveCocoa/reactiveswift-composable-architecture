...
/// then you can create effects by making use of any of  ReactiveSwift's `SignalProducer`s, and then
/// erasing the producer type to ``Effect`` with either `eraseToEffect` or `catchToEffect`. Note
/// that the ReactiveSwift interface to ``Effect`` is considered soft deprecated, and you should
