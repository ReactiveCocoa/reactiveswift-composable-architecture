...
/// ```
///
/// See ``EffectProducer/run(priority:operation:catch:file:fileID:line:)`` for more information on how to
...
extension EffectProducer {
  ...
  @inlinable
  public func map<T>(_ transform: @escaping (Action) -> T) -> EffectProducer<T, Failure> {
...
extension EffectProducer {
  ...
  ///
  /// > Important: This ReactiveSwift-based interface has been soft-deprecated in favor of Swift
  ...
  ///   let playAlertSound: () -> EffectProducer<Never, Never>
  /// }
  /// ```
  ///
...
public typealias Effect = EffectProducer
