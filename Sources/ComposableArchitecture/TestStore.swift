...
extension TestStore where ScopedState: Equatable {
  ...
  /// analytics event in a ``EffectProducer/fireAndForget(priority:_:)`` when an action is sent,
  ...
  func reduce(into state: inout State, action: TestAction) -> EffectTask<TestAction> {
    ...
    case .producer, .run:
      ...
      return
        ...
        .producer
        .on(
          starting: { [effectDidSubscribe, weak self] in
          ...
          interrupted: { [weak self] in self?.inFlightEffects.remove(effect) }
        )
